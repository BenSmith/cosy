...
@test "--sudo enables passwordless sudo access" {
    run "${COSY_SCRIPT}" create --sudo "$TEST_CONTAINER"
    assert_success
    ...
    run podman exec --user "$username" "$TEST_CONTAINER" sudo -n whoami
    assert_success
    assert_output_contains "root"
}

@test "sudo flag is stored in container label" {
    "${COSY_SCRIPT}" create --sudo "$TEST_CONTAINER"

    run podman inspect --format '{{index .Config.Labels "cosy.sudo"}}' "$TEST_CONTAINER"
    assert_success
    assert_output_contains "true"
}

@test "sudo is preserved during recreate" {
    "${COSY_SCRIPT}" create --sudo "$TEST_CONTAINER"
...
@test "container works without --sudo flag" {
    run "${COSY_SCRIPT}" create "$TEST_CONTAINER"
    assert_success

    # User should exist but sudo should require password
    local username
    username=$(podman exec "$TEST_CONTAINER" printenv COSY_CONTAINER_USER)
...
@test "COSY_SUDO environment variable enables sudo" {
